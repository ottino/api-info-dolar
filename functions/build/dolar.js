...
// functions/dolar.js
var dolar_exports = {};
__export(dolar_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(dolar_exports);
...
function mergeConfig(config1, config2) {
  ...
  utils_default.forEach(Object.keys(Object.assign({}, config1, config2)), function computeConfigValue(prop2) {
    ...
    const configValue = merge4(config1[prop2], config2[prop2], prop2);
    utils_default.isUndefined(configValue) && merge4 !== mergeDirectKeys || (config[prop2] = configValue);
  });
  return config;
}
...
// functions/dolar.js
var URL2 = "https://www.infobae.com/economia/divisas/dolar-hoy/";
async function handler(event, context) {
  try {
    ...
    console.error("Error al obtener el valor del d\xF3lar Libre:", error);
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "Error al obtener el valor del d\xF3lar Libre" })
    };
  }
}
async function getDolarLibre() {
  try {
    ...
    const dolarLibreElement = $2(".exchange-dolar-title").filter(function() {
      return $2(this).text().trim() === "D\xF3lar Libre";
    });
